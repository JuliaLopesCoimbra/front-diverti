...
const BRAND_NAMES: Record<string, string> = {
  brahma: "Brahma", sicoob: "Sicoob", volkswagen: "Volkswagen", ballantines: "Ballantine's", globo: "Globo",
};
      ...
      <Box sx={{ px: { xs: 2, sm: 3, md: 4 }, pt: { xs: 3, sm: 4 }, pb: 6 }}>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr 1fr", sm: "repeat(3, 1fr)" }, gap: 2, mb: 4 }}>
          {[
            ...
            { label: "Ativos",   value: MOCK_USERS.length },
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
          {MOCK_USERS.map((u) => (
            <Paper
              key={u.id}
              elevation={0}
              onClick={() => router.push(`/pages/admin-master/usuarios/${u.brand}`)}
              sx={{
                ...
                "&:hover": { backgroundColor: "rgba(255,255,255,0.07)", borderColor: "rgba(255,255,255,0.15)", transform: "translateY(-1px)" },
              }}
            >
              <Avatar src={BRAND_PHOTOS[u.brand]} sx={{ width: 44, height: 44, border: "2px solid rgba(255,255,255,0.1)", flexShrink: 0 }} />
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.8 }}>
                  <Typography sx={{ color: "#fff", fontWeight: 700, fontSize: "0.9rem" }}>{u.name}</Typography>
              ...
              <Chip
                label="Ativo"
                size="small"
                sx={{ backgroundColor: "rgba(16,185,129,0.12)", color: "#10b981", fontWeight: 700, fontSize: "0.68rem", flexShrink: 0 }}
              />
            </Paper>
          ))}
        </Box>
